...
background_image = pygame.transform.scale(pygame.image.load("Wallpaper.jpg"),
...
class Sprite(pygame.sprite.Sprite):
    def __init__(self, color, height, width):
        ...
        pygame.draw.rect(self.image,color, pygame.Rect(0, 0, width,
        height))
        self.rect.x = max(
            min(self.rect.x + x_change, SCREEN_WIDTH - self.rect.width),
        0)
        self.rect.y =max(
            min(self.rect.y + y_change, SCREEN_HEIGHT - 
        self.rect.height), 0)
...
while running:
    ...
    if not won:
        ...
        x_change =  (keys[pygame.K_RIGHT]
                      -keys[pygame.K_LEFT]) * MOVEMENT_SPEED
        y_change =  (keys[pygame.K_DOWN]
                      -keys[pygame.K_UP]) * MOVEMENT_SPEED
    ...
    if won:
        win_text = font.reader("You win!", True, pygame.Color('black'))
